...
The i686 musl package keeps target-specific compatibility adjustments: it enables vendored OpenSSL for static TLS, builds vendored OpenSSL with the lock-based atomic fallback for Zig's 32-bit musl linker, clears restored `openssl-sys` target artifacts so cached builds pick up those C flags, forces `lzma-sys` to compile its bundled liblzma for i686 instead of accepting the runner host's pkg-config archive, compiles the locked `rusty_v8` crate from source as real 32-bit x86 with Chromium's matching bundled Clang/Compiler-RT toolchain, restores and verifies both the ICU data blob and Chromium Rust vendor tree from the exact submodule revisions pinned by that Rusty V8 tag, applies audited i686 C++/Rust layout and cppgc-alignment repairs for `v8 149.2.0` and `v8 150.4.0`, enables Rusty V8 150.4.0's upstream x86-musl branch and `i686-unknown-linux-musl` Rust target while keeping its snapshot toolchain explicitly glibc, injects Zig's i686 musl C headers directly into Rusty V8 bindgen using LLVM 21, force-includes musl's real `stdint.h`, suppresses cargo-zigbuild's later automatic bindgen arguments, compiles the final V8/ICU/libc++ target objects with Chromium Clang's matching builtin headers and Zig's musl libc headers after libc++'s C compatibility wrappers, enables Chromium libc++'s musl configuration, and isolates the runnable 32-bit `mksnapshot` generator in a glibc host toolchain, keeps JavaScript code mode enabled, raises the `codex-mcp-server` recursion limit to 256 for current upstream release builds, and normalizes Linux sandbox syscall constants for 32-bit musl compilation. The bundled CA file comes from the Linux runner's system `ca-certificates` package and is recorded in the manifest with its SHA-256.
